...
As described in \autoref{sec:target_design}, decay radiation emitted from the activated stainless steel foils were used to develop radiochromic film (\textred{Gafchromic EBT3}), revealing the spatial profile of the beam entering and exiting the stack.
Radiochromic films, such as \textred{Gafchromic EBT3}, come in multiple varieties, depending on the   dose range and the type of ionizing radiation desired to provide sensitivity to. 
...
For reference, the \textred{Gafchromic EBT3} film used in this work is comprised of a 28\,\mmicro m active layer containing a dye sensitive to ionizing radiation, sandwiched in between a pair of 125\,\mmicro m layers of clear polyester, acting as a supportive and protective backing. 
...
Following exposure, the film may be digitized using any flatbed scanner.

\cite{Andre2011,David2012}
...
A rendering of the HFNG target chamber, as modeled in MCNP6, is seen in \autoref{fig:ipf_vised}.
...
This model is the same described in \autoref{sec:proton_transport} for simulation of neutron transport in the HFNG.
